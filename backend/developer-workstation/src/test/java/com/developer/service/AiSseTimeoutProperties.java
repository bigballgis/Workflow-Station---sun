 ...
 * {@code n8nTimeoutSeconds * 2 * 1000 + 60000}, ensuring the SSE connection
 * outlives the worst-case N8N call duration (including one retry).</p>
 *
...
class AiSseTimeoutProperties {
    ...
    private AiGenerationServiceImpl createService(int n8nTimeoutSeconds) {
        AiGenerationServiceImpl service = new AiGenerationServiceImpl(
                ...
                new ObjectMapper(),
                102400);
        ReflectionTestUtils.setField(service, "n8nTimeoutSeconds", n8nTimeoutSeconds);
        return service;
    }
     ...
     *
     * <p>For any {@code n8nTimeoutSeconds} in [10, 600], the emitter timeout
     * must equal {@code n8nTimeoutSeconds * 2 * 1000 + 60000}.</p>
     *
     * <p><b>Validates: Requirements 24</b></p>
     */
    @Property(tries = 100)
    @Label("Property 16: emitter.getTimeout() == n8nTimeoutSeconds * 2 * 1000 + 60000")
    void emitterTimeoutMatchesFormula(
            @ForAll @IntRange(min = 10, max = 600) int n8nTimeoutSeconds) {

        AiGenerationServiceImpl service = createService(n8nTimeoutSeconds);

        SseEmitter emitter = service.createChatEmitter(1L, "user1");

        long expectedTimeout = (long) n8nTimeoutSeconds * 2 * 1000 + 60_000L;
        assertThat(emitter.getTimeout())
                .as("Timeout for n8nTimeoutSeconds=%d should be %d ms", n8nTimeoutSeconds, expectedTimeout)
