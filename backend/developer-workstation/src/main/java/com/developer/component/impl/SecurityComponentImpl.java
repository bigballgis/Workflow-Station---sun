...
public class SecurityComponentImpl implements SecurityComponent {
    ...
    public boolean hasRole(String username, String role) {
        log.debug("ROLE CHECK START username={}, role={}", username, role);
        ...
        try {
            ...
            if (cachedResult.isPresent()) {
                boolean result = cachedResult.get();
                log.debug("ROLE CHECK CACHE HIT username={}, role={}, result={}", username, role, result);
                if (auditLogger != null) {
                    Map<String, String> metadata = Map.of("from_cache", "true");
            ...
            // Cache miss - log it
            log.debug("ROLE CHECK CACHE MISS username={}, role={}, querying database...", username, role);
            
            // Query database for role
            boolean hasRole = roleRepository.hasRole(username, role);
            
            log.debug("ROLE CHECK DATABASE RESULT username={}, role={}, hasRole={}", username, role, hasRole);
