...
/**
 * 调用 admin-center 内部接口 redeem SSO code（须配置与 admin 一致的 SSO_INTERNAL_TOKEN）。
 */
@Slf4j
@Component
public class AdminCenterSsoClient {
    ...
    public SsoRedeemResult redeemPortalCode(String code) {
        ...
        try {
            ...
            return new SsoRedeemResult(String.valueOf(m.get("userId")),
                    m.get("username") != null ? String.valueOf(m.get("username")) : "");
        } catch (RestClientException e) {
            log.warn("SSO redeem call failed: {}", e.getMessage());
    ...
    public record SsoRedeemResult(String userId, String username) {
    }
}
