...
export const LOGOUT_EVENT_KEY = "ws_ac_logout_event";
...
export const exchangeSsoCode = async (
  ...
  const response = await authRequest.post<LoginResponse>("/sso/exchange", {
    code,
    state,
  });
  return response.data;
...
// Clear authentication-related localStorage and optionally broadcast to other tabs.
export const clearAuthBroadcast = (broadcast = true) => {
  // Remove user profile cache
  localStorage.removeItem(USER_KEY);
  localStorage.removeItem(USER_ID_KEY);
  localStorage.removeItem(USERNAME_KEY);

  // Tokens are stored in httpOnly cookies; optionally notify other tabs.
  if (broadcast) {
    try {
...
// Backwards-compatible default clearAuth keeps name but broadcasts and clears user as well.
export const clearAuth = (broadcast = true) => {
  localStorage.removeItem(TOKEN_KEY);
  localStorage.removeItem(REFRESH_TOKEN_KEY);
  clearAuthBroadcast(broadcast);
};

// onLogout callback that consuming code (e.g. main.ts) can set.
let onLogoutCallback: (() => void) | null = null;
export const setOnLogout = (cb: () => void) => {
  onLogoutCallback = cb;
};

// Listen for logout events from other tabs and clear local state, then call onLogout.
window.addEventListener("storage", (e: StorageEvent) => {
  if (!e.key) return;
  if (e.key === LOGOUT_EVENT_KEY) {
    // Do not broadcast again when handling an incoming event
    clearAuthBroadcast(false);
    try {
