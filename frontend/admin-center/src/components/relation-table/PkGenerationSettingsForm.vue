<!--
  Mirror: frontend/developer-workstation/src/components/designer/PkGenerationSettingsForm.vue
  Keep Sequence reset + custom format fields in sync; only i18n keys differ (form.* vs table.*).
-->
<template>
  <el-form
    label-position="top"
  ...
  >
    ...
    <el-form-item
      v-if="showPrefix"
      :label="t('form.pkGenerationPrefix')"
    >
      <el-input
    ...
    <el-form-item
      v-if="showReset"
      :label="t('form.pkGenerationResetPeriod')"
    >
      <el-select
      ...
      >
        <el-option
          :label="t('form.pkReset_none')"
          value="none"
        />
        <el-option
          :label="t('form.pkReset_day')"
          value="day"
          :disabled="!canDailyReset"
        />
        <el-option
          :label="t('form.pkReset_month')"
          value="month"
    ...
    <el-form-item
      v-if="showPadWidth"
      :label="t('form.pkGenerationPadWidth')"
    >
      <el-input-number
    ...
    <el-form-item
      v-if="showFormat"
      :label="t('form.pkGenerationFormat')"
    >
      <el-input
