<template>
  <el-dialog
    ...
    width="800px"
  >
    <el-table :data="versions" stripe v-loading="loading" empty-text="No versions found">
      <el-table-column prop="versionNumber" label="Version" width="90" align="center">
        <template #default="{ row }">v{{ row.versionNumber }}</template>
      </el-table-column>
      <el-table-column prop="deployedBy" label="Deployed By" width="140" />
      <el-table-column prop="deployedAt" label="Deployed At" width="180">
        <template #default="{ row }">
          {{ formatDate(row.deployedAt) }}
        </template>
      </el-table-column>
      <el-table-column prop="changeLog" label="Change Log" min-width="200">
        <template #default="{ row }">
          {{ row.changeLog || '-' }}
        </template>
      </el-table-column>
