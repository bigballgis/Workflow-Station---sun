<template>
  <div class="form-list-sidebar">
    <div class="designer-toolbar">
      ...
      <el-button @click="$emit('importFromTable')" :disabled="!hasTables">
        <el-icon><Connection /></el-icon> {{ $t('form.importFields') }}
      </el-button>
    </div>
    
    <el-table :data="forms" v-loading="loading" stripe @row-click="(row: any) => $emit('selectForm', row)">
      ...
      >
        <template #default="{ row }">
          <div class="action-buttons">
            <el-button link type="primary" @click.stop="$emit('selectForm', row)">{{ $t('common.edit') }}</el-button>
            <el-button link type="danger" @click.stop="$emit('deleteForm', row)">{{ $t('common.delete') }}</el-button>
            <el-dropdown trigger="click" @command="(cmd: string) => $emit('moreAction', cmd, row)">
