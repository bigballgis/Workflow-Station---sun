<template>
  <div class="row-formula-editor">
    <el-divider content-position="left">{{ t('businessLogic.rowFormulas') }}</el-divider>
    <div v-for="(formula, idx) in formulas" :key="'rf-' + idx" class="formula-row">
      <el-select v-model="formula.targetColumn" :placeholder="t('businessLogic.targetColumn')" size="small" style="width: 130px">
      ...
      />
      <el-tag v-for="dep in formula.dependsOn" :key="dep" size="small" type="info">{{ dep }}</el-tag>
      <el-button link type="danger" size="small" @click="removeFormula(idx)">{{ t('common.delete') }}</el-button>
    </div>
    <el-button size="small" @click="addFormula">{{ t('businessLogic.addRowFormula') }}</el-button>

    <el-divider content-position="left">{{ t('businessLogic.summaryRules') }}</el-divider>
    <div v-for="(sr, idx) in summaries" :key="'sr-' + idx" class="summary-row">
      <el-select v-model="sr.sourceColumn" :placeholder="t('businessLogic.sourceColumn')" size="small" style="width: 130px">
        <el-option v-for="c in columns" :key="c" :label="c" :value="c" />
      </el-select>
      <el-select v-model="sr.targetField" :placeholder="t('businessLogic.mainFormField')" size="small" style="width: 140px">
        <el-option v-for="f in mainFormFields" :key="f" :label="f" :value="f" />
      </el-select>
      <el-select v-model="sr.aggregation" :placeholder="t('businessLogic.aggregation')" size="small" style="width: 100px">
        <el-option v-for="agg in aggregations" :key="agg" :label="agg" :value="agg" />
      </el-select>
      <el-button link type="danger" size="small" @click="removeSummary(idx)">{{ t('common.delete') }}</el-button>
    </div>
    <el-button size="small" @click="addSummary">{{ t('businessLogic.addSummaryRule') }}</el-button>
  </div>
</template>
