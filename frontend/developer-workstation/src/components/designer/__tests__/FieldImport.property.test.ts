...
describe('Field Import Property Tests', () => {
  ...
  const fieldDefinitionArb = fc.record({
    ...
    length: fc.option(fc.nat().filter(n => n <= 4000), { nil: undefined }),
    precision: fc.option(fc.nat().filter(n => n <= 38), { nil: undefined }),
    scale: fc.option(fc.nat().filter(n => n <= 10), { nil: undefined }),
    nullable: fc.boolean(),
