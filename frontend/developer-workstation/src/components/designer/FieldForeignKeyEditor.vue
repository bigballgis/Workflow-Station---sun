    ...
    >
      ...
      <div class="fk-popover-body">
        ...
        <el-form
          ...
          size="small"
        >
          <el-form-item :label="t('table.refTable')">
            <el-select
              v-model="localRefTableId"
              clearable
              filterable
              style="width: 100%;"
              :placeholder="t('table.refTable')"
              @change="onRefTableChange"
            >
              <el-option
                v-for="tb in refTables"
                :key="tb.id"
                :label="tb.tableDisplayName || tb.tableName"
                :value="tb.id"
              />
            </el-select>
          </el-form-item>
          <el-form-item :label="t('table.refPkFields')">
            <el-select
              ...
              :placeholder="t('table.refPkFields')"
              @change="emitChange"
            >
              <el-option
...
import type { FieldDefinition, TableDefinition } from '@/api/functionUnit'

const props = defineProps<{
  isForeignKey?: boolean
  refTableId?: number | null
  refPrimaryKeyFields?: string[]
  refTables: TableDefinition[]
  refPkFieldOptions: FieldDefinition[]
}>()
...
const summaryLabel = computed(() => {
  if (!localRefTableId.value) return t('table.fkConfigure')
