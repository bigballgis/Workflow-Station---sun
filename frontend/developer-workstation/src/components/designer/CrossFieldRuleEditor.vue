<template>
  <div class="cross-field-rule-editor">
    <div v-for="(rule, idx) in rules" :key="idx" class="rule-row">
      <el-select v-model="rule.fields[0]" :placeholder="t('businessLogic.field1')" size="small" style="width: 130px">
        <el-option v-for="f in fields" :key="f" :label="f" :value="f" />
      </el-select>
      <el-select v-model="rule.operator" :placeholder="t('businessLogic.selectOperator')" size="small" style="width: 130px">
        <el-option v-for="op in operators" :key="op.value" :label="op.label" :value="op.value" />
      </el-select>
      <el-select v-model="rule.fields[1]" :placeholder="t('businessLogic.field2')" size="small" style="width: 130px">
        <el-option v-for="f in fields" :key="f" :label="f" :value="f" />
      </el-select>
      <el-input
        ...
        style="width: 200px"
      />
      <el-select v-model="rule.targetField" :placeholder="t('businessLogic.errorTarget')" size="small" style="width: 130px">
        <el-option v-for="f in fields" :key="f" :label="f" :value="f" />
      </el-select>
      <el-button link type="danger" size="small" @click="removeRule(idx)">{{ t('common.delete') }}</el-button>
    </div>
    <el-button size="small" @click="addRule">{{ t('businessLogic.addCrossFieldRule') }}</el-button>
  </div>
</template>
