    ...
    <div class="diagram-toolbar">
      ...
      <div class="diagram-actions">
        <el-button
          ...
          @click="expandAll"
        >
          {{ t('table.relationExpandAll') }}
        </el-button>
        <el-button
          size="small"
          @click="collapseAll"
        >
          {{ t('table.relationCollapseAll') }}
        </el-button>
        <el-button
          size="small"
          @click="relayout"
        >
          {{ t('table.relationAutoLayout') }}
        </el-button>
      </div>
    </div>
              ...
              >
                ...
                <span class="er-field-badges">
                  <el-tag
                    ...
                    effect="dark"
                    disable-transitions
                  >{{ t('table.relationPkBadge') }}</el-tag>
                  <el-tag
                    v-if="field.isForeignKey"
                    size="small"
                    type="success"
                    disable-transitions
                  >{{ t('table.relationFkBadge') }}</el-tag>
                </span>
...
import type { TableDefinition, FieldDefinition } from '@/api/functionUnit'
import {
...
/** Relation row shape shared with TableDesigner (ids non-null once persisted/derived). */
...
function tableFields(table: TableDefinition): DiagramField[] {
  return (table.fieldDefinitions || []).map((f: FieldDefinition) => ({
  ...
  const built = props.tables.map(table => {
    ...
    return {
      ...
      data: {
        ...
        displayName: table.tableDisplayName || table.tableName,
        expanded,
        visibleFields,
        hiddenCount,
      },
...
function handleConnect(connection: Connection) {
  ...
  if (aTableId === bTableId) {
    ElMessage.warning(t('table.relationSelfRefError'))
    return
  }
  ...
  if (isPkField(bTableId, bField)) {
    ...
    ElMessage.warning(t('table.relationTargetNotPkError'))
    return
  }
...
async function handleEdgeClick({ edge }: { edge: Edge }) {
  ...
  try {
    await ElMessageBox.confirm(
      t('table.relationDeleteConfirm', { field: relation.sourceFieldName }),
      t('table.relationConfig'),
      { type: 'warning' },
    )
  } catch {
    return
