...
export interface ApFlowRun {
  id: string
  flowId: string
  status: ApFlowRunStatus
  startTime?: string | null
  finishTime?: string | null
  flowVersion?: { displayName?: string } | null
  /** Per-step outputs; present on the populated single-run response. */
  steps?: Record<string, unknown> | null
}

/** List runs for the project (optionally one flow), newest first, cursor-paged. */
export async function listAutomationFlowRuns(params: {
  token: string
  projectId: string
  flowId?: string
  cursor?: string
  limit?: number
}): Promise<ApSeekPage<ApFlowRun>> {
  const res = await apAxios.get('/flow-runs', {
    headers: authHeaders(params.token),
    params: {
      projectId: params.projectId,
  ...
  return res.data as ApSeekPage<ApFlowRun>
}

/** One populated run, including its `steps` detail. */
export async function getAutomationFlowRun(runId: string, token: string): Promise<ApFlowRun> {
  const res = await apAxios.get(`/flow-runs/${runId}`, { headers: authHeaders(token) })
