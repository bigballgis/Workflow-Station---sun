...
const ensureMainPrimaryKey = async () => {
  ...
  // Prefer FormRenderer's live model — parent v-model can lag (watchThrottled 150ms),
  // especially for multi LOOKUP which does not go through handleFieldChange.
  const live = (formRendererRef.value as { getFormData?: () => Record<string, unknown> } | null)
    ?.getFormData?.() ?? formData.value
  ...
  if (value != null) {
    const next = { ...live, [pkField.fieldName]: value }
    // If the new PK feeds the Request ID, refresh it so the submitted value includes it.
    if (fieldFeedsRequestId(pkField.fieldName, requestIdConfig.value)) {
      next[REQUEST_ID_FIELD] = computeRequestId(next, requestIdConfig.value) ?? ''
    }
    formData.value = next
  }
}
