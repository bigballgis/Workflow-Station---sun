...
export function isDspAuthenticateConfigured(): boolean {
  return !!import.meta.env.VITE_DSP_AUTHENTICATE_URL
}
...
export async function acquireAmToken(): Promise<AmTokenResult> {
  const url = import.meta.env.VITE_DSP_AUTHENTICATE_URL
  if (!url) {
  ...
  const headers: Record<string, string> = {
    ...
    'Accept-API-Version': import.meta.env.VITE_DSP_ACCEPT_API_VERSION || 'protocol=1.0,resource=2.1',
  }
  const clientId = import.meta.env.VITE_DSP_CLIENT_ID
  if (clientId) headers['X-Client-Id'] = clientId
  const clientSecret = import.meta.env.VITE_DSP_CLIENT_SECRET
