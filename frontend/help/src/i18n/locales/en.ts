...
export default {
  ...
  upTasksToClaimGuide: {
    ...
    filePreviewBody:
      'Click a file name on the form, a sub-table cell, or a note attachment to open preview in a new browser window, so the form stays visible. If the browser blocks the window, preview stays in a dialog. Images and TIFF have Zoom in, Zoom out, 100%, and Fit. Fit is the default. Hold Ctrl (⌘ on Mac) and scroll the mouse wheel to zoom around the cursor. 100% shows the original pixels and uses scroll bars. TIFF ignores thumbnail IFDs so a scanned page is not shown as a tiny preview. When the request has several previewable files, Previous file and Next file stay in the same window and walk main-table uploads, then each sub-table row, then nested sub-tables. Note attachments walk only that note’s files.',
    filePreviewSample: 'new window; Ctrl + scroll zoom; Fit / 100%; Previous file / Next file',
