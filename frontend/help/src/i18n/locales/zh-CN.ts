...
export default {
  ...
  upTasksToClaimGuide: {
    ...
    filePreviewBody:
      '点击表单上的文件名、子表单元格或备注附件，会在新的浏览器窗口打开预览，表单不被挡住。若浏览器拦截了新窗口，则仍使用对话框。图片和 TIFF 提供缩小、放大、100%、适应窗口。默认适应窗口。按住 Ctrl（Mac 上为 ⌘）并滚动鼠标滚轮，会以指针位置为中心缩放。100% 按原始像素显示，超出部分用滚动条查看。TIFF 会忽略缩略图 IFD，避免把扫描件当成很小的图。同一条申请有多个可预览文件时，可在预览页用上一个/下一个文件切换，顺序为主表上传、再按子表行、再嵌套子表。备注附件只在该条备注的文件之间切换。',
    filePreviewSample: '新窗口；Ctrl + 滚轮缩放；适应窗口 / 100%；上一个/下一个文件',
    detailTitle: '任务详情页',
    detailBody:
