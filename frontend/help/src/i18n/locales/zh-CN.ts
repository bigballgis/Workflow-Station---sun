...
export default {
  ...
  guides: {
    ...
    environmentVariables: {
      title: '环境变量',
      summary: '管理中心环境变量目录：TEXT 当前/默认值，以及邮件连接密码用的 VAULT 路径。',
    },
    upTasksToClaim: {
      title: '待办 — 角色认领',
      summary:
  ...
  environmentVariablesGuide: {
    ...
    crumb: '管理中心',
    intro:
      '管理中心 → 环境变量保存 TEXT 取值和 VAULT 密钥路径。邮件连接密码只绑定 VAULT 变量键，邮箱密码留在 Vault。',
    flowTitle: '操作顺序',
    flow1: '新建 VAULT 变量并填写 Vault 密钥路径',
    flow2: '在「连接」里把密码选成该 VAULT 键',
    flow3: '测试发信或部署入站监听',
    catalogTitle: '目录字段',
    catalogBody:
      '打开管理中心左侧的「环境变量」。TEXT 用于普通字符串（默认值必填、当前值可选），VAULT 用于 Vault 密钥路径。邮件密码必须是 VAULT — TEXT 不会出现在连接的密码下拉里。',
    catalogLead: '字段目录 — 新增/编辑变量。',
    fKindText: 'TEXT。默认值必填。当前值可选；空白或只有空格时用默认值。',
    fKindVault: 'VAULT。只填密钥路径。运行时从 HashiCorp Vault 读取 data.data.password。',
    fKey: '必填。稳定键（如 email.qq.inbound.password）。连接和 ZIP 只存这个键，不存密文。',
    fDisplayName: '必填。会出现在连接下拉的显示名。',
    fDefault: 'TEXT 必填。当前值为空或空白时使用。',
    fCurrent: 'TEXT 选填。非空白当前值优先于默认值。',
    fPath: 'VAULT 必填。Vault 密钥路径，如 workflow/email/qq-inbound。不要粘贴密码。',
    bindTitle: '绑到连接',
    bindBody:
