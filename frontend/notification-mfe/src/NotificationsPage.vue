...
const loadNotifications = () => store.fetchNotifications(getQueryParams())

const handleTabChange = () => { currentPage.value = 1; loadNotifications() }
const handlePageChange = () => loadNotifications()

const handleClick = async (item: NotificationData) => {
  try {
    if (!item.isRead) await store.markAsRead(item.id)
    // Note: router navigation to item.link is handled by the host shell
  } catch { /* silent */ }
}
...
const getIcon = (type: string) => {
  const map: Record<string, any> = {
    TASK: Document, PROCESS: Bell, SYSTEM: Setting,
    REMINDER: Warning, PERMISSION: Key, APPROVAL: CircleCheck
  }
...
const getIconColor = (type: string) => {
  const map: Record<string, string> = {
    TASK: 'var(--success-green)', PROCESS: 'var(--warning-orange)',
    SYSTEM: 'var(--info-blue)', REMINDER: 'var(--error-red)',
    PERMISSION: 'var(--info-blue)', APPROVAL: 'var(--warning-orange)'
  }
...
.notifications-page {
  ...
  .notification-list {
    ...
    .notification-item {
      ...
      &:hover { background-color: var(--background-light); }
      &.unread {
      ...
      .notification-icon {
        ...
        background: var(--background-light); border-radius: 50%;
      }
      .notification-content {
        flex: 1;
        .notification-title { font-size: 14px; color: var(--text-primary); margin-bottom: 4px; }
        .notification-desc { font-size: 13px; color: var(--text-secondary); margin-bottom: 8px; }
        .notification-time { font-size: 12px; color: var(--text-placeholder); }
      }
