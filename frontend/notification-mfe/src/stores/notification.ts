...
export const useNotificationStore = defineStore('notification', () => {
  ...
  const { connected, connect, disconnect } = useNotificationWebSocket((n) => {
  ...
  const initWebSocket = () => {
    ...
    watch(connected, (c) => {
      wsConnected.value = c
      c ? stopPolling() : startPolling()
    }, { immediate: true })
    return () => { stopPolling(); disconnect() }
  }

  return { notifications, loading, total, unreadCount, wsConnected, fetchNotifications, fetchUnreadCount, markAsRead, markAllAsRead, deleteNotification, initWebSocket, disconnect }
})
