...
任务整理会复查本轮 Plan 和将要固化的现有设计。有缺陷时 AI 必须写出【设计缺陷提醒】（出处、证据、按原设计会怎样），停等你选：仍按原设计 / 先改 Plan / 纳入本轮修设计。确认过的 Plan 仍可被质疑；不要默认「已确认就可以改代码」。
...
| 阶段 | 人 | AI |
|------|----|----|
| **任务整理** | 写想法 + `按 playbook 整理`；若有【设计缺陷提醒】，选仍按原设计 / 先改 Plan / 纳入本轮 | 输出任务整理块（含【设计缺陷提醒】无或有），**停止**，等确认 |
