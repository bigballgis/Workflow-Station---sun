...
#### 数据库客户端: DBeaver（推荐，如果使用云数据库）

**下载地址**: https://dbeaver.io/download/

**安装步骤**:
1. 下载 DBeaver Community Edition（免费版）
2. 运行安装程序，使用默认选项
3. 启动 DBeaver
4. 首次启动时，DBeaver 会自动下载 PostgreSQL 驱动（如果连接 PostgreSQL）

**用途**:
- 连接云数据库（AWS RDS、Azure Database、阿里云 RDS 等）
- 执行 SQL 脚本初始化数据库
- 查看和管理数据库结构
- 导入/导出数据

**其他选项**: pgAdmin 4（PostgreSQL 官方工具）
...
### 方法 1: 使用 DBeaver 连接云数据库（推荐，如果数据库在云上）

如果您的数据库在云上（如 AWS RDS、Azure Database、阿里云 RDS、腾讯云等），可以使用 DBeaver 直接连接。

#### 步骤 1: 在 DBeaver 中创建云数据库连接
...
3. **填写云数据库连接信息**
   ...
   **示例（不同云服务商）**:
   ...
   - **Azure Database**: `your-server.postgres.database.azure.com`
   - **阿里云 RDS**: `rm-xxxxx.pg.rds.aliyuncs.com`
   - **腾讯云**: `xxxxx.pg.tencentcdb.com`

4. **配置 SSL（如果云数据库要求）**
   - 切换到 "SSL" 标签页
   - 如果云数据库要求 SSL，勾选 "Use SSL"
   - 选择 SSL 模式（通常是 "require" 或 "verify-ca"）
...
#### 步骤 2: 在 DBeaver 中执行 SQL 脚本初始化数据库

如果云数据库是全新的，需要执行 Flyway 迁移脚本来创建表结构：

1. **打开 SQL 编辑器**
   - 在 DBeaver 中，右键点击刚创建的数据库连接
...
2. **按顺序执行迁移脚本**
   ...
   **第一步：执行 platform-security 模块（必须最先执行）**
   - 打开文件：`backend/platform-security/src/main/resources/db/migration/V1__init_schema.sql`
   - 全选内容（`Ctrl+A`）
   - 复制并粘贴到 DBeaver SQL 编辑器
   - 执行（`F5` 或 `Ctrl+Enter`）
   - 检查执行结果，确保没有错误

   **第二步：执行 workflow-engine-core 模块**
   - 打开文件：`backend/workflow-engine-core/src/main/resources/db/migration/V1__init_schema.sql`
   - 复制并执行

   **第三步：执行 admin-center 模块**
   - 打开文件：`backend/admin-center/src/main/resources/db/migration/V1__init_schema.sql`
   - 复制并执行

   **第四步：执行 developer-workstation 模块**
   - 打开文件：`backend/developer-workstation/src/main/resources/db/migration/V1__init_schema.sql`
   - 复制并执行

   **第五步：执行 user-portal 模块**
   - 打开文件：`backend/user-portal/src/main/resources/db/migration/V1__init_schema.sql`
   - 复制并执行
...
#### 步骤 3: 从源数据库导出数据（如果有现有数据）

如果源数据库也有数据需要迁移：

1. **在源数据库连接中导出数据**
   - 在 DBeaver 中，右键点击源数据库连接
   - 选择 "工具" → "转储数据库"
   - 选择要导出的表（或全选）
   - 选择导出格式（SQL 脚本）
   - 点击 "开始" 导出
...
spring:
  datasource:
    url: jdbc:postgresql://your-cloud-db-host:5432/workflow_platform
...
spring:
  datasource:
    url: jdbc:postgresql://your-cloud-db-host:5432/workflow_platform?sslmode=require
    username: your_cloud_username
    password: your_cloud_password
    driver-class-name: org.postgresql.Driver
```

### 方法 2: 使用数据库备份文件（如果数据库在本地 Docker）

如果您的数据库在本地 Docker 容器中：
...
### 方法 3: 使用 Flyway 重新初始化（如果没有数据需要保留）
...
spring:
  datasource:
    url: jdbc:postgresql://your-cloud-db-host:5432/workflow_platform?sslmode=require
    username: your_cloud_username
    password: your_cloud_password  # 建议使用环境变量
    driver-class-name: org.postgresql.Driver
```

**如果使用本地 Docker 数据库**（默认配置）:
```yaml
spring:
  datasource:
...
```env
SPRING_DATASOURCE_URL=jdbc:postgresql://your-cloud-db-host:5432/workflow_platform?sslmode=require
SPRING_DATASOURCE_USERNAME=your_cloud_username
...
**如果使用云数据库（Azure Database、AWS RDS 等）**:

```powershell
# 只启动 Redis 和 Kafka（不需要 PostgreSQL）
...
**⚠️ 重要提示**:
- 如果使用云数据库，**不要**启动 `postgres` 服务
- 确保已更新所有 `application.yml` 中的数据库连接配置指向云数据库
- 后端服务会直接连接到云数据库，不需要本地 PostgreSQL
...
### 3. Maven 下载依赖慢或依赖解析错误

**问题**: `project build error: non-resolvable import POM` 或下载依赖很慢

**解决方案**:

#### 方案 1: 配置国内镜像源（推荐）

编辑 `C:\Users\<用户名>\.m2\settings.xml`（如果不存在则创建）：
...
**详细解决方案**: 参考 [Maven 依赖解析错误修复指南](./maven-dependency-resolve-error-fix.md)
...
1. **在 DBeaver 中测试连接**
   - 打开 DBeaver，尝试连接云数据库
   - 如果 DBeaver 能连接，说明网络和认证没问题

2. **检查云数据库安全组/防火墙规则**
   - **AWS RDS**: 检查安全组，确保允许来自您 IP 地址的连接
   - **Azure Database**: 检查防火墙规则，添加客户端 IP 地址
   - **阿里云 RDS**: 检查白名单设置
   - **腾讯云**: 检查安全组规则

3. **检查数据库配置**
   - `application.yml` 中的数据库 URL 是否正确（包含主机、端口、数据库名）
   - 用户名密码是否匹配
   - SSL 配置是否正确（如果云数据库要求 SSL）

4. **检查网络连接**
   ```powershell
   # 测试网络连通性
   Test-NetConnection -ComputerName your-cloud-db-host -Port 5432
   ```

5. **检查 SSL 证书（如果使用 SSL）**
   - 确保 JDBC URL 中包含正确的 SSL 参数
   - 例如：`?sslmode=require` 或 `?sslmode=verify-ca`
