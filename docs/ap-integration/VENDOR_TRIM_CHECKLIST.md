...
| VT-23 | **web billing 面**：平台侧栏 Billing 项（CE 恒锁）+ `/platform/setup/billing[/success|/error]` 路由页 + `features/billing`(1.8K LOC) + 三处 layout 的 CLOUD-gated `PurchaseExtraFlowsDialog` + dashboard 侧栏 CLOUD-only usage 盒 + chat credits-banner 的 billing 深链 | P2 | ✅ **已完成（2026-08-07，[D13](DECISIONS.md#d13)）**：整树删除；`queryClient` 402 处理收敛为通用 toast，**HERMES-PATCH L1 #6 的 `disableBillingDialogs` 开关随之删除**（弹窗本体没了，开关失去对象；DW 宿主从未显式传它，mount-builder 内部硬编码的 `true` 一并摘除）；`tsc` 0 新增 error（唯一红仍是 `prompt-kit/markdown.tsx` 既有项，stash 基线核实） |
