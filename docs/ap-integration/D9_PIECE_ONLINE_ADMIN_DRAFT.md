...
| D9-3 | **供应链约束**:tarball 必须是仓库 CI `build-piece` 产物;导入时记录 SHA-256、上传人、时间(审计)。**prod 加严:仅接受与 uat 已导入记录(或 git `automation/hermes/tarballs/` 留档)哈希一致的包**——prod 不是新代码的首发环境,只接受 uat 已验证的同一物料。**依赖闭包由离线 store 机械强制**:依赖超出 framework 闭包则 `--offline` 安装 fail-loud,无静默拉包路径 | P3 offline store 固有性质 + 审计记录 |
...
- [ ] **供应链校验落地形态**:哈希比对的权威源选 git 留档(`automation/hermes/tarballs/`)还是 uat 审计表;
      是否需要 CI 签名;
