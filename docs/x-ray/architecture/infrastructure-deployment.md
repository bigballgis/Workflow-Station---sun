...
flowchart TB
    ...
    DW -->|"AI_GATEWAY_URL https://&lt;group-ai-gateway&gt;/chat/completions<br/>Bearer = per-user DSP AMToken"| AIGW[Group AI gateway]
...
| AI Generate (DW) | `AI_GENERATION_ENABLED`, `AI_GATEWAY_URL`, `AI_GATEWAY_MODEL`, `AI_GATEWAY_TIMEOUT_SECONDS`, `DSP_AM_TOKEN_NAME`, plus the gateway FQDN inside `SSRF_ALLOWED_HOSTS` | DW only. No AI credential: the Bearer is the caller's own DSP AMToken, forwarded per request from the browser. |
