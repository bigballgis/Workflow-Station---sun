...
| `deploy/pieces/Dockerfile` | activepieces 0.84.0 | Pre-installs allowlisted pieces from `deploy/pieces/tarballs/*.tgz` at build time (offline/IKP policy) + web patches (`patch-web-approvals.js`, `patch-piece-ai-run-agent.js`); metadata seeded via `deploy/pieces/metadata/pieces-seed.sql` |
...
3. **Activepieces**: `ap-bootstrap-job.yaml` (or `deploy/scripts/ap-bootstrap-shared-account.js`) signs up the shared owner account idempotently (non-prod); pieces pre-baked in the image + `pieces-seed.sql`; flows imported per-env via Jenkins `ap-flows-publish` → new flowId backfilled into `AI_GENERATION_WEBHOOK_URL` and BPMN `ap:flowId`.
