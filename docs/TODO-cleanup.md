...
### 2026-01-22
- 完成：sys_departments 表清理
  - ✅ 从数据库删除 sys_departments 表
  - ✅ 从 sys_users 表删除 department_id 列和 fk_user_department 外键
  - ✅ 从 workflow_platform_executable_clean.sql 删除所有 sys_departments 引用（68行）
  - ✅ 更新 docs/database-flyway-consistency-report.md 标记为已清理
- 待清理：DataPermissionManagerComponent 中的 department_id 字符串引用
  - `backend/admin-center/src/main/java/com/admin/component/DataPermissionManagerComponent.java`
  - 第 79 行：`case DEPARTMENT -> String.format("department_id = '%s'", context.getCurrentDeptId());`
  - 第 92 行：`return String.format("department_id IN (%s)", deptList);`
  - 建议：将 `department_id` 改为 `business_unit_id`，将 `getCurrentDeptId()` 改为 `getCurrentBusinessUnitId()`
  - 注意：这是数据权限过滤功能，需要确保 DataScopeType.DEPARTMENT 类型仍然有效
