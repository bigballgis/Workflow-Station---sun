...
function pkgOf(file) {
    // pnpm store layout: node_modules/.pnpm/<@scope+name>@<ver>/... ('+' joins scoped names)
    let m = file.match(/\.pnpm\/((?:@[^+/]+\+)?[^@/]+)@/)
    if (m) {
        const name = m[1].replace('+', '/')
        // distinguish full zod from zod/mini
        if (name === 'zod') return /\/mini\//.test(file) || /\/v4\/mini\//.test(file) ? 'zod/mini' : 'zod(full)'
        return name
    }
