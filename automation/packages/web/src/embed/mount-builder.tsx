...
export function mountApBuilder(config: MountBuilderConfig): () => void {
  ...
  const hostConfig: ApHostConfig = {
    ...
    portalContainer:
      config.container instanceof HTMLElement ? config.container : undefined,
    embedding: {
      isEmbedded: true,
      hideSideNav: true,
      hideFlowsPageNavbar: true,
      hidePageHeader: true,
      // 0.88 additions to EmbeddingState: the host chrome owns search and
      // presence, so keep both AP surfaces out of the embedded builder.
