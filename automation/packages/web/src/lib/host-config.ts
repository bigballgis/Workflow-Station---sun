...
// HERMES-PATCH-023: upstream returned cloud.activepieces.com when the
// Vite mode was 'cloud' (their hosted dev target). No such mode exists in this
// fork's configs, and an air-gapped deployment must never resolve an off-site
// origin, so the standalone default is unconditionally the serving origin.
