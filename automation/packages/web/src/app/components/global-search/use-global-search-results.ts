...
export function useGlobalSearchResults(query: string, open: boolean) {
  ...
  // HERMES FR-D2: tables domain removed — drop any historical table entries.
  const accessHistory = getAccessHistory().filter((h) => h.type !== 'table');
  ...
  const isSearchLoading =
    flowsQuery.isLoading && searchEnabled;
  ...
  if (!hasQuery) {
    ...
    const isFallbackLoading =
      flowsQuery.isLoading && suggestionsEnabled;
