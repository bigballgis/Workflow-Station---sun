...
export function FunctionSearchPopover({
  ...
  if (!query) {
    return createPortal(
      ...
      apHost.getPortalContainer() ?? document.body,
    );
  }
...
// HERMES-PATCH-023: DEFAULT_FORMULAS_DOCS_URL (upstream formulas docs) removed.
// `docsUrl` is now host-supplied only (embedState.formulasDocsUrl); when the
// embedding host does not provide one the popover simply omits the "See All"
// footer link instead of shipping a dead upstream link.
