...
export const FlowStepInputOutput = () => {
  ...
  const [requestedTab, setActiveTab] = useState<RunActiveTab>('output');
  const activeTab: RunActiveTab =
    requestedTab === 'timeline' || (requestedTab === 'input' && isTrigger)
      ? 'output'
      : requestedTab;
...
const TruncatedInputNotice = () => (
  <div className="flex items-start gap-2 p-3 bg-muted rounded-md text-sm">
    ...
    <span>
      {/* HERMES-PATCH-023: "Learn more" link to upstream troubleshooting docs
          removed; the notice itself is kept because it explains the truncation. */}
      {t(
