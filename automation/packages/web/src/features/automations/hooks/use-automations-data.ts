...
export function useAutomationsData(
  ...
  const folderContentsQuery = useQuery<FolderContentsMap>({
    ...
    queryFn: async () => {
      ...
      // HERMES FR-D2: tables domain removed — folder contents are flows only.
  ...
  // HERMES FR-D2: tables domain removed — keep the shape, always empty.
  const rootTablesQuery = { data: emptyTablePage(), isLoading: false };
