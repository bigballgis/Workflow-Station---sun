    ...
    <Form {...form}>
      <form
        ...
        onSubmit={form.handleSubmit((values) => {
          // HERMES: POST /v1/projects/:id validates with a `.strict()` body
          // (server/api/src/app/project/project.controller.ts UpdateProjectRequestBody)
          // that accepts only releasesEnabled/displayName/externalId/metadata/icon.
          // `globalConnectionExternalIds` is not among them, and it was sent on every
          // submit (the default is [] when nothing is selected), so saving project
          // settings always came back 400.
          mutate({
            projectId,
            request: {
