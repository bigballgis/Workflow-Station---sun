...
export const flowHooks = {
  ...
  // HERMES-PATCH-024: useFetchNpmPackageVersion removed — it did a live browser
  // GET to registry.npmjs.org, which no air-gapped deployment can reach (X-3), and
  // it had zero call sites. The surviving npm entry point (AddNpmDialog) stays: it
  // is already gated off by the upstream ALLOW_NPM_PACKAGES_IN_CODE_STEP flag,
  // which the D6 sandbox baseline (AP_EXECUTION_MODE=SANDBOX_CODE_ONLY) resolves to
  // false in every environment we ship — verified against the running dev instance.
  createFlowQueryKeys: ({
    flowId,
    versionId,
  }: {
