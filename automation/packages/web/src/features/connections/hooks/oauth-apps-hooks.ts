...
export const oauthAppsQueries = {
  ...
  // HERMES-PATCH-023: the cloud-hosted OAuth app map (secrets.activepieces.com)
  // is gone, so `cloudOAuth2App` is always null and only platform-managed OAuth
  // apps from `/v1/oauth-apps` populate the map. The CLOUD_OAUTH2 branches in the
  // connection dialog therefore never render — no silent degradation, the entry
