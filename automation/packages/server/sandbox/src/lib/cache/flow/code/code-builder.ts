...
export const codeBuilder = (log: ApLogger, getSettings: () => SandboxSettings) => ({
    async processCodeStep({
        ...
        const { cacheHit } = await cache.getOrSetCache({
            ...
            // A transient dependency-install failure must self-heal: never cache the throwing stub,
            // so the next build re-runs install. Deterministic compile errors stay cached. See GIT-1608.
