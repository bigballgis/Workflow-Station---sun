        ...
        async resolve(input: ResolveInput): Promise<ResolveResult> {
            ...
            if (!isNil(input.flow)) {
                ...
                if (resolved.code.kind === 'source') {
                    ...
                    // code step built cleanly — a transient dependency-install failure must never be
                    // baked into the shared flow bundle (GIT-1608). A failed compile/publish never
                    // fails the run; the pool recompiles from source.
