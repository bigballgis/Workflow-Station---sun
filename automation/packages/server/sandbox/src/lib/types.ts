...
// builds `${publicApiUrl}v1/engine/pieces/bundle?name=&version=&token=` per piece, downloads the
// tarball (following the endpoint's redirect to npm / signed-S3, or streaming the custom archive)
// and hands the local bundle.tgz to the pnpm install (HERMES: upstream handed the URL to bun, since
// removed — see piece-installer.ts). No bytes cross the worker socket and the pool never imports
// WorkerToApiContract; the link is publicApiUrl-based so it is reachable from a remote pool
// (Cloud Run). See ADR 0002.
