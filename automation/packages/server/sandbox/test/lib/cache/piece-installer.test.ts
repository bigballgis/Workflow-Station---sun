...
describe('pieceInstaller', () => {
    ...
    // Regression: another pnpm run against the shared workspace can empty a member's
    // node_modules without removing the directory (a filtered install prunes every sibling
    // member when the prewarm and the runtime disagree on the pnpm version). `ready` survives
    // that, so a check on the bare directory reported "installed" forever and the piece could
    // never be recovered — every flow run failed with PieceNotFound. The probe is the package
    // path the engine's loader resolves, so this state reinstalls instead.
    it('ready marker but the package is gone from node_modules — reinstalls and clears ready', async () => {
        const piece = makePiece('@activepieces/piece-purged')
        const pieceDir = pieceDirPath(piece)
    ...
    // The check must hit the disk on every provision, not answer from a snapshot taken earlier
    // in the process: the purge that empties node_modules happens WHILE the worker is running
    // (another install against the shared workspace), so an in-memory "installed" memo keeps
    // the process serving PieceNotFound until it is restarted.
    it('purged after a successful install in the same process — reinstalls without a restart', async () => {
        const piece = makePiece('@activepieces/piece-revisit')
        const installer = pieceInstaller(fakeLog, testWorkspace, fakeGetSettings)
    ...
    // A pnpm isolated-linker install leaves node_modules/<name> as a symlink into
    // node_modules/.pnpm. fileExists uses access(), which follows it — so a link whose target
    // was pruned reads as missing rather than as a healthy install.
    it('node_modules/<name> is a dangling symlink — reinstalls', async () => {
        const piece = makePiece('@activepieces/piece-dangling')
        const pieceDir = pieceDirPath(piece)

        await mkdir(join(pieceDir, 'node_modules', '@activepieces'), { recursive: true })
        await symlink(join(pieceDir, 'node_modules', '.pnpm', 'gone'), join(pieceDir, 'node_modules', piece.pieceName))
        await writeFile(join(pieceDir, 'ready'), 'true')
    ...
    it('archive piece installs through pnpm with a unique suffixed workspace name pointing at its bundle link', async () => {
        ...
        // cached versions of the same piece can never collide on a workspace member name.
    ...
    it('skips pieces whose name is a relative path — they never reach the shared workspace', async () => {
        const good = makePiece('@activepieces/piece-good')
        // Stale `usedPieces` data from a since-reverted build can carry a relative path as the
        // pieceName. Writing it as a workspace member corrupts the shared workspace lockfile and breaks every
    ...
    it('install made up only of invalid-named pieces is a no-op — pnpm never runs', async () => {
        const poison = makePiece('../../../common/pieces/@activepieces/piece-algolia', '0.0.3')
        const installer = pieceInstaller(fakeLog, testWorkspace, fakeGetSettings)
    ...
    it('mixes valid and invalid pieces — only valid ones reach pnpm, both filters present for valid', async () => {
