...
export const flowOperation = {
    execute: async (operation: ExecuteFlowOperation): Promise<EngineResponse<undefined>> => {
        ...
        // HERMES-PATCH-007: release a caller blocked on the sync webhook as soon as the run
        // reaches a terminal state, instead of making it wait out WEBHOOK_TIMEOUT_MS.
        await syncWebhookRelease.onRunSettled({
            ...
            status: finished.verdict.status,
        })
