...
// HERMES 0.88 移植：0.84 经 worker socket 发布，0.88 引擎改走 HTTP 回调 engineRunApi。
...
describe('HERMES-PATCH-007 — sync webhook 终态释放', () => {
    const CALLER = { apiUrl: 'http://api.internal/', engineToken: 'engine-token' }
    ...
    it.each(NON_TERMINAL)('非终态 %s 不释放 —— 响应要等 resume 之后才来', async (status) => {
        await syncWebhookRelease.onRunSettled({ ...CALLER, ...IDS, status })
    ...
    it('没有 workerHandlerId / httpRequestId 就没人在等，不发', async () => {
        ...
        await syncWebhookRelease.onRunSettled({ ...CALLER, workerHandlerId: null, httpRequestId: null, status })
        await syncWebhookRelease.onRunSettled({ ...CALLER, workerHandlerId: 'handler-1', httpRequestId: null, status })
        await syncWebhookRelease.onRunSettled({ ...CALLER, workerHandlerId: null, httpRequestId: 'req-1', status })

        expect(mockSendFlowResponse).not.toHaveBeenCalled()
    })

    it('发布失败不外溢 —— run 已报完状态，超时仍是兜底', async () => {
        mockSendFlowResponse.mockRejectedValue(new Error('callback failed'))

        await expect(
            syncWebhookRelease.onRunSettled({ ...CALLER, ...IDS, status: FlowRunStatus.FAILED }),
