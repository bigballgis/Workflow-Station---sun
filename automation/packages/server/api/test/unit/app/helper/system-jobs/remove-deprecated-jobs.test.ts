...
describe('removeDeprecatedJobs', () => {
    ...
    it('removes deprecated schedulers and their delayed jobs while keeping live ones', async () => {
        ...
        await seedQueue.upsertJobScheduler(SystemJobName.PIECES_SYNC, { pattern: '0 * * * *', tz: 'UTC' }, { name: SystemJobName.PIECES_SYNC, data: {} })
        ...
        expect(schedulerNames).toEqual([SystemJobName.PIECES_SYNC])
        expect(jobNames).toEqual([SystemJobName.PIECES_SYNC])
    })
})
