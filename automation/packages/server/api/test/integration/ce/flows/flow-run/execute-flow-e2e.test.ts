 ...
 *
 * Prerequisites (all three are steps in .github/workflows/ap-api-tests.yml — read its header
 * before running this by hand, especially the pnpm-version trap):
 *   - Engine must be built: `turbo run build --filter=@activepieces/engine` writes
 *     dist/packages/engine/main.js, which engine-installer.ts copies into cache/v13/common
 *   - The four pieces pinned below must be prewarmed into cache/v13/common:
 *     `AP_PREWARM_WORKSPACE="$PWD/cache/v13/common" sh hermes/prewarm-pieces.sh \
 *        "$PWD/hermes/pieces.e2e-fixtures.json"` — with the SAME pnpm the runtime installer
 *     spawns (the package.json `packageManager` pin), or the prewarm gets pruned silently
 *   - `node_modules/.bin` on PATH: the CODE step spawns a bare `esbuild` (pkg-runner.ts)
...
// `worker.stop()` is async and does real work — it drains in-flight jobs, shuts the runtime
// down and disconnects the socket. Not awaiting it left app.close() racing a live socket and
// live jobs, so teardown blew the 15s hook budget on a slower/loaded machine while every test
// in the file had passed. Observed as `1 failed | 51 passed (52)` files against
// `372 passed (372)` tests, which is a confusing way to learn about a missing await.
afterAll(async () => {
    await worker.stop()
    await app.close()
}, 30_000)
