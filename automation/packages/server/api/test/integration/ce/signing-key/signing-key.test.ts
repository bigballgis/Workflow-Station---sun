...
// HERMES: CE has no platform_plan row (getPlan always returns OPEN_SOURCE_PLAN), so the
// signing-key endpoints are no longer plan-gated — the platform setup needs no override.
const setupEnabledPlatform = () => mockAndSaveBasicSetup()
