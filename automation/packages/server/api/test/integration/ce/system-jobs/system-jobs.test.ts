...
afterEach(async () => {
    ...
    for (const s of schedulers) {
        ...
        if (key.startsWith(TEST_PREFIX) || key.includes('::') || key === 'pieces-sync') {
...
describe('System Jobs', () => {
    ...
    it('should keep new-format schedulers while removing legacy ones', async () => {
        ...
        const legacyKey = `${SystemJobName.PIECES_SYNC}::0:UTC:0 12 * * *`
        await systemJobsQueue.upsertJobScheduler(legacyKey, {
            pattern: '0 12 * * *',
            tz: 'UTC',
        }, {
        ...
        await systemJobsQueue.upsertJobScheduler('pieces-sync', {
            pattern: '0 12 * * *',
            tz: 'UTC',
        }, {
            name: SystemJobName.PIECES_SYNC,
        ...
        const before = await systemJobsQueue.getJobSchedulers()
        const analyticsBefore = before.filter(s => s.name === SystemJobName.PIECES_SYNC)
        ...
        const after = await systemJobsQueue.getJobSchedulers()
        const legacyAfter = after.filter(
            s => s.name === SystemJobName.PIECES_SYNC && s.key.includes('::'),
        )
