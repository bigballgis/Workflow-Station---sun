...
import { FlowVersion, PopulatedTriggerSource, TriggerSource } from '@activepieces/shared'
...
export const triggerSourceService = (log: FastifyBaseLogger) => {
    return {
        async enable(params: EnableTriggerParams): Promise<TriggerSource> {
            const { flowVersion, projectId, simulate, isRepublish } = params
            log.info({
            ...
            // HERMES: template domain removed (AG-EE); template telemetry (ACTIVATE) dropped.
            // params.templateId stays in the contract — it is flow metadata, not telemetry.
        ...
        async disable(params: DisableTriggerParams): Promise<void> {
            ...
            // HERMES: template domain removed (AG-EE); template telemetry (DEACTIVATE) dropped.
        },
    }
}
