...
export const setupServer = async (): Promise<FastifyInstance> => {
    ...
    // HERMES-PATCH-015 (0.88): the MCP OAuth root controllers are deleted with
    // the mcp domain (FR-D2) — no MCP client exists in the air gap.
    // HERMES-PATCH-021 (0.88): the OIDC discovery pair
    // (GET /.well-known/openid-configuration, GET /.well-known/jwks.json) is
    // deleted for the same reason. It was registered with no prefix, i.e. at the
    // domain root, and served the workload-identity token endpoint that is also
    // gone; no relying party exists inside the air gap.
