...
export const flowFolderService = (log: FastifyBaseLogger) => ({
    ...
    async list(params: ListParams): Promise<SeekPage<FolderDto>> {
        ...
        const queryBuilder = folderRepo()
            ...
            // HERMES: tables domain removed (AG-EE); FolderDto still carries numberOfTables
            // (shared contract), so it is a constant 0 — no "table" table exists to count.
            .addSelect('0::int', 'numberOfTables')
    ...
    async getOneOrThrow(params: GetOneOrThrowParams): Promise<FolderDto> {
        ...
        return {
            ...folder,
            numberOfFlows,
            // HERMES: tables domain removed (AG-EE); constant 0, no tables exist in CE.
            numberOfTables: 0,
        }
    },
})
