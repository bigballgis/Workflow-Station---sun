...
export const authenticationUtils = (log: FastifyBaseLogger) => ({
    async assertUserIsInvitedToPlatformOrProject({
        ...
        // HERMES: user-invitations domain removed (AG-EE / EE_REMOVAL_PLAN); users are
        // provisioned via the HERMES managed-authn handshake, so no invitation can exist.
        // With open sign-up disabled a direct sign-up is always rejected — fail loud
        // instead of silently admitting the user.
        log.warn({ email, platform: { id: platformId } }, 'Sign-up rejected: invitations are removed in HERMES CE')
