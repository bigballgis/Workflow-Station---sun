...
async function removeDeprecatedJobs(log: FastifyBaseLogger): Promise<void> {
    const deprecatedJobs = [
        ...
        // HERMES: upstream 0.88 job names whose domains this fork deleted. They were left
        // in SystemJobName with zero registerJobHandler calls, which put them in
        // `knownJobNames` below and therefore exempted any scheduler a pre-trim deployment
        // had already written to Redis — where it would fire forever and fail in
        // getJobHandler. Listing them here purges the stragglers.
        'pieces-analytics',
        'hard-delete-project',
