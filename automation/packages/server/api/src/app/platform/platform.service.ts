import { apId, isNil, PlatformId, spreadIfDefined, spreadIfNotUndefined, UserId } from '@activepieces/core-utils'
import { AuthenticationResponse, OPEN_SOURCE_PLAN, Platform, PlatformPlanLimits, PlatformRole, PlatformUsage, PlatformWithoutFederatedAuth, PlatformWithoutSensitiveData, ProjectType, SsoDomainVerification, UpdatePlatformRequestBody, UserStatus } from '@activepieces/shared'
...
export const platformService = (log: FastifyBaseLogger) => ({
    ...
    async create(params: AddParams): Promise<PlatformWithoutFederatedAuth> {
        ...
        // HERMES: EE platform-plan bootstrap removed (AG-EE / EE_REMOVAL_PLAN G6).
...
async function getBillingEnforced(_log: FastifyBaseLogger, _platformId: PlatformId): Promise<boolean | undefined> {
    // HERMES: billing enforcement was CLOUD/ENTERPRISE-only (the EE autumn provider is gone
    // with app/ee). COMMUNITY already short-circuited to undefined, which is now the only path.
    return undefined
}
