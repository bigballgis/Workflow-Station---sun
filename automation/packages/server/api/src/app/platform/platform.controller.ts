...
export const platformController: FastifyPluginAsyncZod = async (app) => {
    ...
    app.get('/:id', GetPlatformRequest, async (req) => {
        ...
        if (req.principal.type === PrincipalType.USER) {
            // HERMES: chatVisibilityHelper removed with the ee agent/chat domain (FR-A03/FR-D2).
            // The embedded licenseKey masking is unrelated to chat and is kept.
    ...
    // HERMES: EE CLOUD-only platform deletion endpoint removed (AG-EE / EE_REMOVAL_PLAN G6).
    // It depended on platform-plan (subscription check), platform-teardown-jobs and the EE
    // email service, all of which went with app/ee.
}
