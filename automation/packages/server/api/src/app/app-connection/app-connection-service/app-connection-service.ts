...
// HERMES: EE project-member service replaced by the CE reimplementation (AG-EE / EE_REMOVAL_PLAN G2).
import { projectMemberService } from '../../project/project-member.service'
// HERMES: EE secret-managers service removed; identity stub keeps call sites intact (AG-EE / EE_REMOVAL_PLAN G4).
import { containsSecretManagerReference, secretManagersService } from '../secret-manager-stub'
...
const validateConnectionValue = async (
    ...
    switch (value.type) {
        ...
        case AppConnectionType.CLOUD_OAUTH2: {
            // HERMES-PATCH-026: refuse CLOUD_OAUTH2 at the API boundary.
            //
            // Its handler POSTs the authorization code (claim) and the refresh token
            // (refresh) to https://secrets.activepieces.com — an Activepieces-owned host.
            // That is the last path by which a credential could leave an air-gapped
            // cluster (X-3), and unlike piece egress it carries OAuth secrets.
            //
            // The web entry point went with HERMES-PATCH-023, but the server still
            // accepted the type, so any direct API call still reached the upstream host.
            // DEPENDENCY_MAP cited `cloudAuthEnabled=false` as the control — that control
            // does not exist: AppSystemProp.CLOUD_AUTH_ENABLED defaults to 'true', is read
            // only by flag.service.ts to publish a flag, and no server code consults it.
            // Refuse explicitly instead of relying on a flag nothing reads.
            //
            // Self-hosted OAuth2 (AppConnectionType.OAUTH2, own client id/secret) and
            // PLATFORM_OAUTH2 are unaffected — they never leave the cluster.
            throw new ActivepiecesError({
                code: ErrorCode.INVALID_APP_CONNECTION,
                params: {
                    error: 'Activepieces-hosted OAuth (CLOUD_OAUTH2) is disabled: it would send the authorization code and refresh token to secrets.activepieces.com. Use a self-hosted OAuth2 connection with your own client credentials.',
                },
            })
        }
