...
export function createHandlers(log: FastifyBaseLogger, assignment: WorkerGroupAssignment | null = null, connectionId?: string): WorkerToApiContract {
    return {
        ...
        // HERMES: EE agents removed (AG-EE). The agent RPC methods are retained to satisfy
        // the WorkerToApiContract (shared) but are never invoked (no agent feature in CE);
        // they fail explicitly if the worker ever calls them — mirrors 0.84's chatRemoved().
        ...
        async heartbeatAgentConversation() {
            throw agentsRemoved()
        },
        ...
        async executePieceTool() {
            throw agentsRemoved()
        },

        async executeKnowledgeBaseTool() {
            throw agentsRemoved()
        },

        async executeFlowTool() {
            throw agentsRemoved()
        },

        async updateFlowStepProgress() {
            throw agentsRemoved()
        },

        async resumeFlowStep() {
            throw agentsRemoved()
        },
...
function agentsRemoved(): Error {
    return new Error('Agents are not available in this deployment')
}
