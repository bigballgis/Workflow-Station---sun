...
export const pieceSyncService = (log: FastifyBaseLogger) => ({
    async setup(): Promise<void> {
        ...
        // HERMES-PATCH-019: upstream registers the hourly cloud-sync cron unconditionally and only
        // checks the mode inside sync(), so a disabled install still schedules a job whose sole
        // output is an hourly "disabled" log line. With the whitelist frozen (C-2) the catalog only
        // ever changes via the seeded pieces-seed.sql, so skip scheduling entirely when disabled.
        if (syncMode !== PieceSyncMode.OFFICIAL_AUTO) {
            log.info({ syncMode }, '[pieceSyncService#setup] Cloud piece sync disabled — cron not scheduled')
            return
        }
    ...
    async sync({ publishCacheRefresh }: { publishCacheRefresh: boolean }): Promise<void> {
        ...
        try {
            ...
            // HERMES: tool-search domain removed (AG-EE); no embedding/index reconcile on catalog change.
        }
        catch (error) {
...
async function installNewPieces(cloudPieces: PieceRegistryResponse[], dbPieces: PieceMetadataOnly[], log: FastifyBaseLogger, _publishCacheRefresh: boolean): Promise<number> {
    ...
    for (let done = 0; done < newPiecesToFetch.length; done += batchSize) {
        ...
        await Promise.all(currentBatch.map(async (piece) => {
            ...
            // HERMES: pnpm resolves undici-types v6, whose Response.json() is Promise<unknown>
            // (bun's hoisted tree gave Promise<any>). Cast at the boundary, shape unchanged.
