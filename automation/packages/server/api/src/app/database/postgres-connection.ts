...
// HERMES: the 20 app/ee/database migrations are deleted with the EE strip
// (EE_REMOVAL_PLAN G16, scheme A — fresh databases only). The unified MIT
// migrations (Unify* et al.) recreate every table the CE rewrites still use.
...
export const getMigrations = (): (new () => Migration)[] => {
    const migrations = [
        ...
        // HERMES: repoint branding/piece icons at locally served /ap-cdn assets
        // (air-gapped prod, DECISIONS X-2/X-3). Ported from the 0.84 fork
        // (1794000000000) and renumbered past the last upstream migration.
        HermesLocalizeCdnAssets1824000000000,
        // HERMES: the piece_set domain is EE and was trimmed, but 1807 still creates
