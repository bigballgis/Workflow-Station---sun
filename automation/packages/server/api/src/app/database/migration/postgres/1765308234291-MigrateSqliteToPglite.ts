...
export class MigrateSqliteToPglite1765308234291 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        try {
            // HERMES-PATCH-020: import sqlite-connection lazily, inside the PGLITE-only branch.
            // It has a top-level `import 'sqlite3'`, so a static import here pulled the whole
            // sqlite3 driver (~34MB) into every image — including POSTGRES deployments, which is
            // all of ours, and which early-return above without ever touching this code. The
            // sqlite→PGLite upgrade path still works; the driver is only required when it runs.
    ...
    // HERMES-PATCH-020: type-only reference via `import()` so it stays erased at runtime —
    // a value import here would defeat the lazy load above and pull sqlite3 back in.
    private async sqliteHasData(sqliteDataSource: ReturnType<typeof import('../../sqlite-connection').createSqlLiteDataSourceForMigrations>): Promise<boolean> {
        try {
