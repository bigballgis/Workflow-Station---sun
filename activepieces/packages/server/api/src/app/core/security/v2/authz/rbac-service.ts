...
// HERMES: CE reimplementation of the project-access RBAC service, moved out of app/ee
// (AG-EE / EE_REMOVAL_PLAN G2). Table-driven per-project authorization semantics are
// preserved 1:1 from upstream: USER access is resolved from project_member -> project_role
// permissions; ENGINE is scoped to its own projectId; SERVICE to its own platform. The
// Q4a shared-project bypass and the JWT-carried-permissions fast path are HERMES
// integration layers to be added on top later (AP UserPrincipal carries no permissions).
...
// HERMES: standalone wrapper preserving the positional signature the flow controller used
// from the old ee rbac-middleware (AG-EE / EE_REMOVAL_PLAN G2/G11). Delegates to the CE
// rbacService; the edition gate inside makes it a no-op in CE.
export async function assertUserHasPermissionToFlow(
    ...
    await rbacService(log).assertUserHasPermissionToFlow({ principal, operationType, projectId })
}

// HERMES: standalone wrapper preserving the positional signature the old ee rbac-middleware
// exposed (AG-EE / EE_REMOVAL_PLAN G2). USER-only gate then delegate to the project access
// check; non-USER principals are allowed (matches upstream behaviour).
export async function assertRoleHasPermission(
    ...
    await rbacService(log).assertPrinicpalAccessToProject({ principal, permission, projectId })
}
