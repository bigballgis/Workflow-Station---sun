...
3. 验证成功的标志:流程实例显示**已完成**,变量 `apResult` 里是 AP 返回的值。

> **纯自动化流程的正常表现**:只有自动化节点、没有人工任务时,它几百毫秒就跑完,门户里看不到"下一步"——这是正常的,不是卡住。

### 想让它在门户里像"正常申请"(有待办、可交互)

在自动化节点**前面加一个用户任务**(在设计器里:拖一个用户任务,接进流程线,派给"发起人/INITIATOR",再给它配一个动作如"提交")。这样:
- 发起申请后,门户「我的待办」里出现这个任务;
- 你打开任务、点"提交" → AP 节点自动跑 → 结果回写 → 流程结束。

> 要让任务里**能填字段、能看到 AP 结果**,得给这个用户任务**绑一个表单**(在设计器里画表单 + 把字段绑到 `inputText` / `apResult`)。这属于正常的功能单元搭建,不在本手册的最小示例范围内。
...
- **发布到生产**:`ap-export.js` 导出 → 提交 git → **Jenkins 流水线**发布(内部跑 `ap-import.js`)→ 生产建 connection + 改 Flow ID
