...
-- 4. Purchase Items Form -> purchase_item (PRIMARY)
INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order)
SELECT 
    fd.id,
    td.id,
...
-- 7. Supplier Selection Popup -> supplier_info (PRIMARY)
INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order)
SELECT 
    fd.id,
    td.id,
    ...
    0
FROM dw_form_definitions fd, dw_table_definitions td
WHERE fd.form_name = 'Supplier Selection'
  AND td.table_name = 'supplier_info'
  AND fd.function_unit_id = (SELECT id FROM dw_function_units WHERE code = 'fu-purchase-request')
  AND td.function_unit_id = fd.function_unit_id
ON CONFLICT (form_id, table_id) DO NOTHING;

-- 8. Budget Query Popup -> budget_info (PRIMARY, READONLY)
INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order)
SELECT 
    fd.id,
    td.id,
    ...
    0
FROM dw_form_definitions fd, dw_table_definitions td
WHERE fd.form_name = 'Budget Query'
  AND td.table_name = 'budget_info'
  AND fd.function_unit_id = (SELECT id FROM dw_function_units WHERE code = 'fu-purchase-request')
  AND td.function_unit_id = fd.function_unit_id
ON CONFLICT (form_id, table_id) DO NOTHING;

-- 9. Countersign Form -> purchase_request (PRIMARY, READONLY)
INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order)
SELECT 
    fd.id,
    td.id,
...
-- 10. Countersign Form -> countersign_record (SUB, EDITABLE)
INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order)
SELECT 
    fd.id,
    td.id,
