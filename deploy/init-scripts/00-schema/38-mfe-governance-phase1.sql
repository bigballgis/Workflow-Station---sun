-- =====================================================
-- MFE Governance Phase 1: Frontend Module Registry
-- =====================================================
-- Configuration-driven MFE governance baseline:
-- Admin Center manages module registry; host apps render
-- nav/routes dynamically from runtime config.
-- =====================================================
...
-- Unique module per tenant + host + env
CREATE UNIQUE INDEX IF NOT EXISTS uk_ac_fm_tenant_host_env_module
  ON ac_frontend_module_registry(tenant_id, host_app, env, module_code);

-- Unique route per tenant + host + env
CREATE UNIQUE INDEX IF NOT EXISTS uk_ac_fm_tenant_host_env_route
  ON ac_frontend_module_registry(tenant_id, host_app, env, route_path);
...
CREATE INDEX IF NOT EXISTS idx_ac_fm_mgmt_list
  ON ac_frontend_module_registry(tenant_id, host_app, env);
