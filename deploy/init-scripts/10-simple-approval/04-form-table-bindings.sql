...
BEGIN
    SELECT id INTO v_function_unit_id FROM dw_function_units WHERE code = 'fu-20260403-a1b2c0';
    IF v_function_unit_id IS NULL THEN
        RAISE EXCEPTION 'Function unit fu-20260403-a1b2c0 not found.';
    END IF;
    ...
    INSERT INTO dw_form_table_bindings (form_id, table_id, binding_type, binding_mode, foreign_key_field, sort_order, created_at, updated_at) VALUES
        ...
        (v_approval_form_id, v_sub_table_id,      'SUB',     'READONLY', 'request_id', 2, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
        (v_approval_form_id, v_action_table_id,   'RELATED', 'EDITABLE', 'request_id', 3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
        (v_approval_form_id, v_relation_table_id, 'RELATED', 'READONLY', 'request_id', 4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
