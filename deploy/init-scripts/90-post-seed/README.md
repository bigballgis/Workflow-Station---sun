...
2. **`01-admin/`, `08-/15-/16-/17-`** — seed data (users / roles, demo
   Function Units).
...
- `99-maintenance/` is wrong: by name and convention these are on-demand
  repair scripts.
- `08-/15-/16-/17-` is wrong: cross-cutting, not tied to one FU.

`90-post-seed/` fills the gap.

## Naming & ordering

- Directory prefix `90-` simply means "comes after the `17-` Function Unit
...
2. Register it in `deploy/init-scripts/00-init-all.sh` in the appropriate
   sub-step (currently `[5e/6]`). The runner is a **whitelist loader**, not
