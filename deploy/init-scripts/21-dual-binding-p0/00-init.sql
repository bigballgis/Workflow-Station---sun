...
-- =============================================================================
-- 21-dual-binding-p0: same-table two SUB bindings (local P0 vehicle)
-- Code: p0-dual-binding-test
-- Tables: p0_dual_case (MAIN), p0_dual_party (SUB), p0_dual_file (SUB, two FKs)
-- Not loaded by 00-init-all. BINDING_EXISTS stays closed; SQL insert is intentional.
-- =============================================================================
...
DECLARE
    ...
    v_bind_primary     BIGINT;
    v_bind_party       BIGINT;
    v_bind_file_case   BIGINT;
    v_bind_file_party  BIGINT;
    v_view_party       BIGINT;
    v_view_file_case   BIGINT;
    v_view_file_party  BIGINT;
    ...
    v_pk_json          JSONB;
BEGIN
    -- Rebuild is destructive for this code only.
    SELECT id INTO v_fu_id FROM dw_function_units WHERE code = 'p0-dual-binding-test';
    ...
    INSERT INTO dw_function_units (
        ...
        'p0-dual-binding-test',
        'P0 Dual Binding Test',
        'Local P0 vehicle: one PROCESS form binds p0_dual_file twice (case_id vs party_id). Unique table names so Demo attachment merger stays unambiguous.',
        'DRAFT',
        '1.0.0', '1.0.0',
        true, true,
        CURRENT_TIMESTAMP, 0,
    ...
    (v_fu_id, 'p0_dual_case', 'P0 Dual Case', 'MAIN', 'Case (MAIN)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    RETURNING id INTO v_case_id;

    INSERT INTO dw_table_definitions (
        function_unit_id, table_name, table_display_name, table_type, display_name, created_at, updated_at
    ) VALUES
    (v_fu_id, 'p0_dual_party', 'P0 Dual Party', 'SUB', 'Parties of the case', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    RETURNING id INTO v_party_id;

    INSERT INTO dw_table_definitions (
        function_unit_id, table_name, table_display_name, table_type, display_name, created_at, updated_at
    ) VALUES
    (v_fu_id, 'p0_dual_file', 'P0 Dual File', 'SUB', 'Files bound twice (case vs party)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    INSERT INTO dw_form_definitions (
        ...
        'P0 Dual Case Form',
        'PROCESS',
        'Start form with parties plus the same file table bound twice',
        v_case_id,
        'TASK',
        '{"rule":[],"options":{"form":{"size":"default","inline":false,"labelWidth":"125px","labelPosition":"left","hideRequiredAsterisk":false},"resetBtn":{"show":false,"innerText":"Reset"},"submitBtn":{"show":true,"innerText":"Submit"}},"subForms":{}}'::jsonb,
    ...
    INSERT INTO dw_form_table_bindings (
        ...
        v_form_id, v_case_id, 'PRIMARY', 'EDITABLE', NULL, 'structuralFk', 0, NULL, NULL,
        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    )
    RETURNING id INTO v_bind_primary;

    INSERT INTO dw_form_table_bindings (
        form_id, table_id, binding_type, binding_mode, foreign_key_field,
        binding_link_mode, sort_order, sub_mode, filter_fk_field_id, created_at, updated_at
    ) VALUES (
        v_form_id, v_party_id, 'SUB', 'EDITABLE', 'case_id', 'structuralFk', 1, 'FULL', v_party_fk_case,
        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    )
    RETURNING id INTO v_bind_party;

    INSERT INTO dw_form_table_bindings (
        form_id, table_id, binding_type, binding_mode, foreign_key_field,
        binding_link_mode, sort_order, sub_mode, filter_fk_field_id, created_at, updated_at
    ) VALUES (
        v_form_id, v_file_id, 'SUB', 'EDITABLE', 'case_id', 'structuralFk', 2, 'FULL', v_file_fk_case,
        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    )
    RETURNING id INTO v_bind_file_case;

    INSERT INTO dw_form_table_bindings (
        form_id, table_id, binding_type, binding_mode, foreign_key_field,
        binding_link_mode, sort_order, sub_mode, filter_fk_field_id, created_at, updated_at
    ) VALUES (
        v_form_id, v_file_id, 'SUB', 'EDITABLE', 'party_id', 'structuralFk', 3, 'FULL', v_file_fk_party,
        CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
    )
    RETURNING id INTO v_bind_file_party;

    INSERT INTO dw_sub_table_view_configs (binding_id, created_at, updated_at)
    VALUES (v_bind_party, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    RETURNING id INTO v_view_party;
    INSERT INTO dw_sub_table_view_configs (binding_id, created_at, updated_at)
    VALUES (v_bind_file_case, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    RETURNING id INTO v_view_file_case;
    INSERT INTO dw_sub_table_view_configs (binding_id, created_at, updated_at)
    VALUES (v_bind_file_party, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    RETURNING id INTO v_view_file_party;

    INSERT INTO dw_sub_table_view_fields (view_config_id, field_name, display_label, column_width, sort_order, visible) VALUES
    (v_view_party, 'id', 'Party ID', 150, 0, true),
    (v_view_party, 'party_name', 'Party name', 200, 1, true),
    (v_view_file_case, 'id', 'File ID', 150, 0, true),
    (v_view_file_case, 'file_name', 'File name', 200, 1, true),
    (v_view_file_party, 'id', 'File ID', 150, 0, true),
    (v_view_file_party, 'file_name', 'File name', 200, 1, true);

    UPDATE dw_form_table_bindings SET sub_list_view_id = v_view_party WHERE id = v_bind_party;
    UPDATE dw_form_table_bindings SET sub_list_view_id = v_view_file_case WHERE id = v_bind_file_case;
    UPDATE dw_form_table_bindings SET sub_list_view_id = v_view_file_party WHERE id = v_bind_file_party;

    UPDATE dw_form_definitions
    SET config_json = jsonb_build_object(
            'rule', jsonb_build_array(
                jsonb_build_object(
                    'name', 'ref_p0_title', 'type', 'input', 'field', 'title',
                    'title', 'Title', '_fc_id', 'id_p0_title', 'hidden', false, 'display', true,
                    '_fc_drag_tag', 'input',
                ...
                jsonb_build_object(
                    'name', 'ref_p0_party', 'type', 'subTable', 'title', 'Parties',
                    '_fc_id', 'id_p0_party', 'hidden', false, 'display', true,
                    '_fc_drag_tag', 'subTable', '_bindingId', v_bind_party, 'props', '{}'::jsonb
                ),
                jsonb_build_object(
                    'name', 'ref_p0_file_case', 'type', 'subTable', 'title', 'Case files',
                    '_fc_id', 'id_p0_file_case', 'hidden', false, 'display', true,
                    '_fc_drag_tag', 'subTable', '_bindingId', v_bind_file_case, 'props', '{}'::jsonb
                ),
                jsonb_build_object(
                    'name', 'ref_p0_file_party', 'type', 'subTable', 'title', 'Party files',
                    '_fc_id', 'id_p0_file_party', 'hidden', false, 'display', true,
                    '_fc_drag_tag', 'subTable', '_bindingId', v_bind_file_party, 'props', '{}'::jsonb
                )
            ),
            ...
            'subForms', jsonb_build_object(
                v_bind_party::text, '{"rule":[{"name":"ref_p0_pn","type":"input","field":"party_name","title":"Party name","_fc_id":"id_p0_pn","hidden":false,"display":true,"_fc_drag_tag":"input","props":{"maxlength":100}}],"options":{"form":{"labelWidth":"125px","labelPosition":"left"},"submitBtn":{"show":true,"innerText":"Submit"},"resetBtn":{"show":false}}}'::jsonb,
                v_bind_file_case::text, '{"rule":[{"name":"ref_p0_fn_c","type":"input","field":"file_name","title":"File name","_fc_id":"id_p0_fn_c","hidden":false,"display":true,"_fc_drag_tag":"input","props":{"maxlength":200}}],"options":{"form":{"labelWidth":"125px","labelPosition":"left"},"submitBtn":{"show":true,"innerText":"Submit"},"resetBtn":{"show":false}}}'::jsonb,
                v_bind_file_party::text, '{"rule":[{"name":"ref_p0_fn_p","type":"input","field":"file_name","title":"File name","_fc_id":"id_p0_fn_p","hidden":false,"display":true,"_fc_drag_tag":"input","props":{"maxlength":200}}],"options":{"form":{"labelWidth":"125px","labelPosition":"left"},"submitBtn":{"show":true,"innerText":"Submit"},"resetBtn":{"show":false}}}'::jsonb
            )
        ),
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" xmlns:custom_1="http://custom.bpmn.io/schema" xmlns:custom="http://workflow.platform/schema/custom" xmlns:flowable="http://flowable.org/bpmn" id="Definitions_P0Dual" targetNamespace="http://bpmn.io/schema/bpmn">
  <bpmn:process id="p0-dual-binding-test" name="P0 Dual Binding Test" isExecutable="true">
    ...
    <bpmn:sequenceFlow id="Flow_2" sourceRef="Task_FillDual" targetRef="EndEvent_1" />
  </bpmn:process>
...
$xml$,
        v_action_submit, v_action_save, v_form_id
    );
    ...
    VALUES (v_form_id, 'Task_FillDual', 'Fill Dual Binding Case', 'TASK');

    IF (SELECT COUNT(*) FROM dw_form_table_bindings
        WHERE form_id = v_form_id AND table_id = v_file_id AND binding_type = 'SUB') <> 2 THEN
        RAISE EXCEPTION 'P0 dual-binding seed failed: expected 2 SUB bindings on p0_dual_file';
    END IF;
    IF v_file_fk_case = v_file_fk_party THEN
        RAISE EXCEPTION 'P0 dual-binding seed failed: filter FK field ids must differ';
    END IF;

    RAISE NOTICE 'p0-dual-binding-test ready: fu=% form=% file_table=% bind_case=% bind_party=% filter_case=% filter_party=%',
        v_fu_id, v_form_id, v_file_id, v_bind_file_case, v_bind_file_party, v_file_fk_case, v_file_fk_party;
END $p0$;
