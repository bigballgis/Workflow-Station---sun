...
-- 各环境适用性：**全部需要手工执行本文件**
-- -----------------------------------------------------------------------------
-- 2026-08 起所有环境的 FLOWABLE_SCHEMA_UPDATE 统一为 false（dev / sit / preprod /
-- uat / prod）。引擎**不会**自动迁移，版本不符会直接启动失败。发布新引擎版本前
-- 必须：停引擎 → 整库备份 → 执行本文件 → 再启动引擎。
--
-- 唯一例外是单元测试（application-test.yml 保持 true）：每次跑都是全新的内存 H2，
-- 没有需要保护的数据。
--
