...
-- 各环境适用性
-- -----------------------------------------------------------------------------
--   dev / sit / preprod : FLOWABLE_SCHEMA_UPDATE=true，引擎启动时**自动迁移**，
--                         通常无需手工执行本文件。若想手工控制，先把该变量设为
--                         false，执行本文件，再启动引擎。
--   uat / prod          : FLOWABLE_SCHEMA_UPDATE=false，引擎**不会**自动迁移，
--                         版本不符会直接启动失败 —— **必须**执行本文件。
--
-- 已验证：本机 dev 库（PostgreSQL 16.5）由引擎自动迁移走完同样三步，
--         结果 schema.version=7.2.0.2、19 个加宽列全部保持、存量流程数据完好。
-- =============================================================================
