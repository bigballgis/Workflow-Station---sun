#!/usr/bin/env python3
"""
Convert PostgreSQL COPY FROM stdin commands to INSERT statements (Version 3)
Correctly handles trailing empty fields and special characters
"""
...
def convert_copy_to_insert(input_file, output_file):
    ...
    def replace_copy(match):
        ...
        for line in lines:
            ...
            # Split by tab - use -1 to keep trailing empty strings
            values = line.split('\t')
            
            # Handle trailing tabs (empty fields at the end)
            # If line ends with tab, split() will not include the final empty string
            # We need to count tabs to determine actual field count
            tab_count = line.count('\t')
            expected_fields = tab_count + 1
            
            # Adjust values list if needed
            while len(values) < expected_fields:
                values.append('')
            
            # Validate column count
            if len(values) != num_columns:
                error_msg = f"Table {table_name}, line {line_num}: Expected {num_columns} columns, got {len(values)} (tabs: {tab_count})"
                ...
                if len(values) < num_columns:
                    ...
                    print(f"    → Fixed by truncating extra fields")
            ...
            for i, value in enumerate(values):
                if value == '\\N' or value == '':
                    ...
                    # In PostgreSQL COPY format, empty string and \N both mean NULL
        ...
        # Split into multiple INSERT statements if too many rows
        batch_size = 100
    ...
    if conversion_stats['errors']:
        print(f"\n⚠️  Warnings: {len(conversion_stats['errors'])} rows had issues (auto-fixed)")
    
    return conversion_stats

if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python convert_copy_to_insert_v3.py <input_file> <output_file>")
        print("Example: python convert_copy_to_insert_v3.py workflow_platform_v2_sqlonly.sql workflow_platform_v2_insert_fixed.sql")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n✅ All done! You can now use {output_file} in DBeaver.")
