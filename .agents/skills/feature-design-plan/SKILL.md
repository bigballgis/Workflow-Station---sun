...
| 执行 | `ai-development-playbook` | 任务整理：允许/禁止路径、最低验证命令 → 确认后改代码。整理时若发现本 Plan 或既有设计有缺陷，写入【设计缺陷提醒】，停等用户选择 |
...
4. **Plan 已确认 ≠ 实现时不可质疑。** playbook 只读整理对照仓库后，若本 Plan（或任务会固化的现有设计）有缺陷，必须在任务整理里写【设计缺陷提醒】（出处、证据、按原设计会怎样、建议选项），停等用户选：仍按原设计 / 先改 Plan 再执行 / 纳入本轮修设计。禁止装没看见，禁止默默改设计。阻塞到写不出诚实【修改计划】时，先 `AskQuestion`（≤2）。详见 playbook「整理时发现既有设计有缺陷」。
